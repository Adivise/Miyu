...
const NormalPage = async (client, message, pages, timeout, queueLength, queueDuration) => {
    ...
    collector.on('collect', async (interaction) => {
            ...
            curPage.edit({ embeds: [pages[page].setFooter({ text: `Page • ${page + 1}/${pages.length} | ${queueLength} • Song/s | ${queueDuration} • Total Duration` })], components: [row] })
        });
    collector.on('end', () => {
        const disabled = new ActionRowBuilder()
            .addComponents(row1.setDisabled(true), row2.setDisabled(true))
        curPage.edit({ embeds: [pages[page].setFooter({ text: `Page • ${page + 1}/${pages.length} | ${queueLength} • Song/s | ${queueDuration} • Total Duration` })], components: [disabled] })
    });
    return curPage;
};
